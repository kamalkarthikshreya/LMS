...
import { ChevronLeft, ChevronRight, Book, ArrowLeft, MessageSquare } from 'lucide-react';
...
const SubjectReader = () => {
    ...
    return (
        <div className="flex h-[calc(100vh-4rem)] bg-slate-50 relative overflow-hidden animate-fade-in">
            ...
            {/* Sidebar Navigation */}
            <div className="w-64 bg-white/80 backdrop-blur-xl border-r border-slate-200/50 overflow-y-auto hidden md:block z-10 shadow-[4px_0_24px_-12px_rgba(0,0,0,0.1)]">
                        ...
                        <div className="px-4 py-10 sm:px-8 md:px-16 lg:px-24">
                            <div className="max-w-3xl mx-auto">
                                <div className="mb-14 animate-fade-in-up text-center">
                                    <span className="inline-flex items-center px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest bg-slate-900 text-white shadow-xl shadow-slate-900/20 mb-6">
                                        Unit {currentUnit?.unitNumber || currentUnitIdx + 1} • Chapter {currentChapter?.chapterNumber || currentChapterIdx + 1}
                                    </span>
                                    <h1 className="text-5xl md:text-6xl font-black text-slate-900 leading-tight tracking-tighter drop-shadow-sm">
                                        {currentSection?.title || 'Untitled Section'}
                                    </h1>
                                    <div className="w-24 h-1.5 bg-gradient-to-r from-primary-500 to-indigo-500 mx-auto mt-8 rounded-full"></div>
                                </div>
                                ...
                                {/* Navigation Controls */}
                                <div className="flex justify-between items-center pb-20 pt-8 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                                    <button
                                        onClick={() => navigateContent('prev')}
                                        disabled={currentUnitIdx === 0 && currentChapterIdx === 0 && currentSectionIdx === 0}
                                        className="btn-secondary gap-2 px-6 py-3 text-slate-600 hover:text-primary-700 shadow-sm"
                                    >
                                        <ChevronLeft size={20} /> Previous Section
                                    </button>
                                    <button
                                        onClick={() => navigateContent('next')}
                                        className="btn-primary gap-2 px-8 py-3 rounded-xl shadow-lg shadow-primary-500/30"
                                    >
